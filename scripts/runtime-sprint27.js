...
/**
 * TEAKLE — Sprint #27 Runtime Tests
 ...
 *   node scripts/runtime-sprint27.js
 */
...
(async () => {
  ...
  const ts = Date.now();
  const custEmail = `sprint27_${ts}@test.com`;
  const custPass = 'TestPass27!';
  ...
  await check('POST /api/auth/register with valid data → 200 or 409', async () => {
    ...
    const r = await fetch(BASE + '/api/auth/register', {
      ...
      body: JSON.stringify({ email: custEmail, password: custPass, confirmPassword: custPass, name: 'Sprint 27 Test' }),
    });
  ...
  await check('PUT /api/auth/profile with valid data → 200', async () => {
    ...
    const r = await fetch(BASE + '/api/auth/profile', {
      ...
      body: JSON.stringify({ name: 'Updated Sprint 27' }),
    });
    if (r.status !== 200) return 'status: ' + r.status;
    const j = await r.json();
  ...
  await check('POST /api/contact with valid data → success', async () => {
    ...
    const r = await fetch(BASE + '/api/contact', {
      ...
      body: JSON.stringify({
        ...
        subject: 'Test Subject', message: 'Test message for sprint 27',
      }),
    });
    const j = await r.json().catch(() => ({}));
