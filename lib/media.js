...
// Server-authoritative extension per validated MIME type (see createMedia).
const EXT_BY_MIME = {
  'image/jpeg': '.jpg',
  'image/png': '.png',
  'image/webp': '.webp',
...
export async function createMedia(file, altText = '') {
  ...
  const { safeName } = saveFile(file.name, buffer, EXT_BY_MIME[file.type]);
...
export function deleteMedia(id) {
  ...
  // Check order item image references
  const orderItemRef = db.prepare(`
    SELECT id FROM order_items WHERE productImage = ?
  `).get(media.url);

  if (orderItemRef) {
    throw new Error('Media is currently referenced by an order item');
  }

  // Check inline references inside CMS section body HTML
  const bodyRef = db.prepare(`
    SELECT id FROM content_sections WHERE body LIKE ?
  `).get(`%${media.url}%`);

  if (bodyRef) {
    throw new Error('Media is currently referenced in CMS content body');
  }
