...
function initSchema(db) {
  ...
  migratePaymentWebhookEvents(db);
  migrateCustomerSessionVersion(db);
}
...
function migratePaymentWebhookEvents(db) {
  db.exec(`CREATE INDEX IF NOT EXISTS idx_payment_webhook_events_provider ON payment_webhook_events(provider)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_payment_webhook_events_receivedAt ON payment_webhook_events(receivedAt)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_payment_webhook_events_processed ON payment_webhook_events(processed)`);
}

function migrateCustomerSessionVersion(db) {
  const cols = db.prepare("PRAGMA table_info(customers)").all();
  const colNames = cols.map(c => c.name);
  if (!colNames.includes('sessionVersion')) {
    db.exec(`ALTER TABLE customers ADD COLUMN sessionVersion INTEGER NOT NULL DEFAULT 0`);
  }
}
