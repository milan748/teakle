...
export async function createCustomerSession(customer) {
  ...
  // Set CSRF cookie (non-HttpOnly, readable by JS for double-submit pattern)
...
export async function getCustomerSession() {
  ...
  try {
    ...
    const db = getDb();
    const customer = db.prepare('SELECT id, isActive FROM customers WHERE id = ?').get(payload.customerId);
