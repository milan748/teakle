...
const PAYMENT_PROVIDER = (process.env.PAYMENT_PROVIDER || 'none').toLowerCase();
const KNOWN_PROVIDERS = ['razorpay', 'stripe', 'none'];

export function getPaymentConfig() {
  return {
    provider: PAYMENT_PROVIDER,
    configured: PAYMENT_PROVIDER !== 'none' && KNOWN_PROVIDERS.includes(PAYMENT_PROVIDER),
  };
}
...
export function createPaymentRecord({ orderId, idempotencyKey, provider = 'none' }) {
  ...
  if (amounts.totalAmount <= 0) {
    log.error('Payment creation failed — order total must be > 0', { orderId, totalAmount: amounts.totalAmount });
    return null;
  }

  const result = db.prepare(`
    INSERT INTO payments (orderId, provider, providerPaymentId, amount, currency, status, idempotencyKey, createdAt, updatedAt)
    VALUES (?, ?, NULL, ?, 'INR', 'UNPAID', ?, datetime('now'), datetime('now'))
...
export async function createPaymentIntent({ orderId, idempotencyKey }) {
  ...
  return {
    ...
    configured: true,
    provider: config.provider,
    reason: 'Provider integration not yet implemented',
    orderId,
    amount: amounts.totalAmount,
    currency: 'INR',
  };
...
export async function verifyPayment({ paymentId, providerPaymentId }) {
  ...
  return {
    ...
    provider: config.provider,
    reason: config.configured ? 'Provider integration not yet implemented' : 'Payment provider not configured',
    paymentId,
  };
}
 ...
 * Validates refund amount bounds before processing.
 */
export async function processRefund({ paymentId, amount, reason }) {
  ...
  if (amount > payment.amount) {
    return { ok: false, error: `Refund amount (${amount}) exceeds payment amount (${payment.amount})` };
  }
 ...
 */
export async function handleWebhook({ provider, signature, body, eventId }) {
  if (!provider) {
  ...
  if (provider.toLowerCase() === 'none') {
    return { ok: false, status: 501, error: 'Payment provider not configured' };
  }

  if (!signature) {
    log.paymentWebhookRejected(provider, 'missing_signature');
    return { ok: false, status: 400, error: 'Missing webhook signature' };
  }
