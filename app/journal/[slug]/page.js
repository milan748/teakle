...
export default function JournalArticlePage() {
  ...
  useEffect(() => {
    ...
    if (found) {
      setArticle(found);
      document.title = `${found.title} — Teakle Journal`;

      // Related articles (same category, exclude current)
      const related = articles
        .filter((a) => a.slug !== slug && a.category === found.category)
        .slice(0, 3);
      // If not enough same-category, fill from others
      if (related.length < 3) {
        const extras = articles
          .filter((a) => a.slug !== slug && a.category !== found.category && !related.find((r) => r.slug === a.slug))
      ...
      // Related products
      if (found.relatedProducts && window.TEAKLE_PRODUCTS) {
        const prods = found.relatedProducts
    ...
    return (
      <>
        ...
        <section style={{ background: 'var(--bg-primary)', padding: 'var(--space-2xl) var(--space-md)', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: 'var(--space-md)' }}>The article you&apos;re looking for doesn&apos;t exist or has been moved.</p>
