...
// In-memory guard that closes the concurrency race between the DB row insert
// (unprocessed) and the final processed=1 update. A duplicate event delivered
// while the first is still in flight is short-circuited here. This complements
// (does not replace) the DB-level UNIQUE(provider, eventId) + processed check.
const inFlightEvents = new Map();

function markEventInFlight(key) {
  inFlightEvents.set(key, Date.now());
  // Bound the map: prune entries older than 10 minutes.
  if (inFlightEvents.size > 500) {
    const cutoff = Date.now() - 10 * 60 * 1000;
    for (const [k, t] of inFlightEvents) {
      if (t < cutoff) inFlightEvents.delete(k);
    }
  }
}
...
export async function POST(req) {
  try {
    ...
    const dedupeKey = `${provider}:${eventIdStr}`;

    // In-memory in-flight guard (closes the concurrent-delivery race window).
    if (inFlightEvents.has(dedupeKey)) {
      log.info('Webhook event already in flight (deduplicated)', { provider, eventId: eventIdStr });
      return NextResponse.json({ ok: true, message: 'Event already processing' });
    }
